...
describe("§17.2 — a trilha de câmera na malha", () => {
  it("ligar a câmera anexa a trilha a TODOS os pares e renegocia com cada um", async () => {
    ...
    // Um `addTrack` de vídeo por par: a câmera é malha, não estrela — não há audiência a
    // filtrar, e quem está na chamada vê.
    for (const pc of criadas) {
      const chamadas = (pc.addTrack as ReturnType<typeof vi.fn>).mock.calls;
      expect(chamadas.some((c) => (c[0] as MediaStreamTrack).kind === "video")).toBe(true);
    }
    expect((porta.signal as ReturnType<typeof vi.fn>).mock.calls.length).toBeGreaterThanOrEqual(2);
  });
  ...
  it("quem ENTRA com a câmera já ligada recebe o vídeo na primeira oferta, sem renegociar", async () => {
    ...
    const chamadas = (nova.addTrack as ReturnType<typeof vi.fn>).mock.calls;
    expect(chamadas.some((c) => (c[0] as MediaStreamTrack).kind === "video")).toBe(true);
    // A oferta inicial já leva o vídeo: `createOffer` uma vez, não duas.
    expect((nova.createOffer as ReturnType<typeof vi.fn>).mock.calls.length).toBe(1);
  });

  it("desligar retira a trilha de cada conexão", async () => {
    ...
    for (const pc of criadas) expect(pc.removeTrack).toHaveBeenCalled();
  });
  ...
  it("a câmera não sobrevive à chamada: sair esquece a trilha", async () => {
    ...
    const nova = criadas[criadas.length - 1]!;
    const chamadas = (nova.addTrack as ReturnType<typeof vi.fn>).mock.calls;
    expect(chamadas.some((c) => (c[0] as MediaStreamTrack).kind === "video")).toBe(false);
  });
});
