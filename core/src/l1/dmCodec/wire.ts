// Primitivos do registry de DM — §31.4. O layout é o de §7.2.1, sem `Scope` e sem `rank`
// (a conversa é o escopo; não há ordenação fracionária). Formato de fio idêntico ao de
// `compact-encoding`.
//
// **Por que estes primitivos são uma segunda implementação, e não um import de `opCodec`.**
// §4 dá a `dmCodec` a coluna "Depende de" vazia, exatamente como a de `opCodec`: são módulos
// irmãos, não extensões um do outro (§31.0). Importar `opCodec` daqui é importação lateral
// não declarada e o `check-layers` do `npm run build` quebra. A independência é deliberada —
// o registry de DM é versionado por `DM_VERSION`, e um bump de `opVersion` não pode arrastar
// a conversa direta junto (§31.4). O preço é esta duplicação, e ela é o preço declarado.
//
// A implementação é própria também por uma razão de totalidade: **o decodificador do
// `dmFold` não pode lançar** (§31.7.1). `compact-encoding` lança em buffer truncado; aqui
// truncamento liga `failed`, que o estágio 1 de §31.7.3 mapeia para `IGNORED` / `E_MALFORMED`.
//
...
// | key              | fixed32                                                       |
// | sig              | fixed64                                                       |
// | str / id         | string (uint prefixado, UTF-8)                               |
// | bool             | uint8 0/1                                                     |
 ...
 * antes de concluir que a entrada é malformada. Não viola a totalidade de §31.7.1, mas é
 * negação de serviço trivial contra qualquer réplica: o estágio 0 de §31.7.3 limita o
 * **registro**, não o que um prefixo mentiroso promete dentro dele.
 */
