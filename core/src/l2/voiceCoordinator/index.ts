 ...
 * fechamento obrigatório de `voice.revoked` e recusa a sessão mesmo com ticket novo.
 */
export class VoiceTicketManager {
  ...
  readonly #revoked = new Set<string>(); // `${sessionId}:${remotePeerHex}`

  constructor(opts: { hostPublicKey: Buffer; localPeer: Buffer; clock?: { now(): number } }) {
  ...
  acceptSignaling(args: { sessionId: string; channelId: string; remotePeer: Buffer; ticket: MediaTicket }): AcceptSignalingResult {
    ...
    if (this.#revoked.has(`${args.sessionId}:${hex}`)) return { ok: false, code: 'E_TICKET_INVALID' };
  ...
  canInitiateDtls(sessionId: string, remotePeer: Buffer): boolean {
    ...
    if (this.#revoked.has(`${sessionId}:${hex}`)) return false;
   ...
   * Devolve a lista de conexões a fechar agora; a sessão fica recusada mesmo com ticket
   * novo, e a renovação cessa (o pior caso sem evento é a expiração do ticket, §17.4).
   */
  ...
  dropSession(sessionId: string): void {
    ...
    for (const key of [...this.#revoked]) if (key.startsWith(`${sessionId}:`)) this.#revoked.delete(key);
  }
