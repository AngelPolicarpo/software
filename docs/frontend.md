...
- **1.4 Popover de perfil de membro** — acionado a partir da lista de membros, do nome/avatar do autor de uma mensagem, ou de um participante de voz. Mesmo componente, vários gatilhos.
...
- Entrar em canal de voz **não troca a área de conteúdo principal** se Ana estava vendo um canal de texto — abre a barra de chamada persistente e, opcionalmente, o usuário pode clicar na barra para expandir a grade de participantes/tela compartilhada como overlay. Sair do canal de voz fecha a barra.
...
| **Botão** | Variantes: primário (`accent`), secundário (superfície elevada + borda), ghost (sem fundo), destrutivo (`feedback-danger`), ícone-only. Estados: default, hover, active, focus (anel `border-strong`), disabled, loading (spinner substitui label). |
...
| **Item de lista de canal** | Estados: default, hover, ativo (fundo `accent-muted-bg`), não-lido (texto mais claro + dot), com menção pendente (badge numérico `feedback-danger`), silenciado (ícone mudo, sem destaque de não-lido), canal de voz com gente dentro (avatares inline dos participantes abaixo do nome). |
...
| **Barra de chamada persistente** | Fixada na base da lista de canais (Desktop/Tablet) ou no rodapé da viewport (Mobile), mostra canal de voz atual + avatares dos participantes + controles rápidos enquanto Ana navega outros canais. Layout e conjunto de controles variam por breakpoint — detalhado em 2.3.1. |
...
**Layout (Desktop, esquerda→direita):** rail de comunidades (72px) · lista de canais (240px) · área de conteúdo (flexível) · painel de membros/thread (280px, um de cada vez, fechado por padrão). Barra de chamada persistente ancorada na base da lista de canais quando há voz ativa.
...
**Conteúdo do rail (topo→base):** ícones das comunidades de Ana na ordem do dataset (§2) — Vale do Código, Clã Noturno, Ateliê Aberto —, separador, botão "+" (criar/entrar), espaço flexível, avatar de Ana no rodapé (abre 3.1).
**Conteúdo da lista de canais:** nome da comunidade ativa no topo (clicável → painel de membros), categorias colapsáveis (ver dataset §2 pra Vale do Código), canal de voz lista participantes ativos inline abaixo do nome. Para quem tem `gerenciar canais`, mais três afordâncias discretas (especificadas em §10, 3.4): "+" no cabeçalho da lista, "+" no header de cada categoria e "+ Nova categoria" no fim da lista.
...
**Feedback visual:** ao confirmar, a menção vira um token não-editável inline no composer (pill compacta — `accent-muted-bg` para cargo/everyone, cor do cargo do membro para pessoa); um único `Backspace` no token remove a menção inteira, não caractere por caractere.
...
**Layout:** ao entrar, a área de conteúdo principal (se Ana estava olhando aquele canal) muda pra grade de participantes; se Ana estava em outro canal de texto, abre como overlay expandido a partir da barra de chamada persistente (§4).
...
É a representação compacta da mesma sessão de voz de 2.3 — existe sempre que Ana está conectada a um canal de voz e não está olhando a grade expandida (por exemplo, navegando um canal de texto, ou até numa comunidade diferente da que hospeda a chamada, per fluxo C11). Layout muda por breakpoint porque o espaço disponível é fundamentalmente diferente (240px fixos no Desktop/Tablet vs. largura total no Mobile) — não é só uma versão "espremida" da mesma barra.

**Desktop e Tablet** (ambos mantêm a coluna de lista de canais de 240px, §16):
- Container: 240px de largura (igual à lista de canais), **64px de altura**, ancorado na base fixa da lista de canais (não do rail — o rail de 72px nunca hospeda a barra). Fundo `surface-elevated` (destaca da `surface-sidebar` por trás — sinaliza camada própria), `border-subtle` separando da lista de canais acima, padding 8px nos quatro lados — a barra é camada própria e mede a partir da própria borda, não se alinha à coluna de texto da lista de canais (que começa 8px mais à direita). Altura fecha em 8 + 20 + 4 + 24 + 8 = 64; os 56px das versões anteriores desta spec nunca couberam com o padding de 8px que ela mesma pedia — sobravam 4px por lado, e como a barra encosta na base da janela, esses 4px eram toda a distância entre os avatares e a borda da tela.
- **Linha 1** (topo, ~20px): ícone 🔊 12px + nome do canal (`text-body-emphasis`, truncado com reticências) +, só quando a chamada pertence a uma comunidade diferente da ativa no momento, " · " + nome da comunidade em `text-tertiary` (também truncado). Sem esse sufixo quando a chamada é da própria comunidade ativa — informação redundante nesse caso.
- **Linha 2** (base, ~28px): à esquerda, stack de avatares sobrepostos (20px cada, overlap ~6px, borda 2px `surface-elevated` pra separar do fundo), máximo 3 visíveis + badge circular "+N" se houver mais participantes. À direita, 3 ícones compactos 20px com 4px de gap: mudo, ensurdecer, sair.
- Clicar em qualquer área da barra fora dos 3 ícones de controle expande pra 2.3 como overlay (§4).

**Mobile** (<640px):
- Container: largura total da viewport, 64px de altura, fixo no rodapé — acima de qualquer uma das 3 telas empilhadas (§16), sempre visível independente de qual está em foco. Fundo `surface-elevated`, `border-subtle` no topo, padding 12px.
- **Uma linha só** (mais espaço horizontal disponível que no Desktop): à esquerda, stack de avatares (24px cada, mesma lógica de overlap/overflow) + ao lado, nome do canal (`text-body-emphasis`) e, numa segunda linha de texto pequena abaixo, nome da comunidade em `text-tertiary` — mesma regra condicional (só mostra se a comunidade da chamada for diferente da ativa). À direita, só 2 controles — **mudo** e **sair** — cada um com área de toque de 44px (convenção de touch target mínimo) mesmo com ícone visualmente menor; ensurdecer/câmera/compartilhar ficam só na vista expandida (2.3), pra não lotar uma barra estreita com alvos de toque pequenos demais.
- Toque em qualquer área fora dos 2 botões expande pra 2.3 em tela cheia.

**Avatares — sempre presentes em ambos os breakpoints.** A barra recolhida é frequentemente a única indicação visual de quem está na chamada (a chamada pode pertencer a uma comunidade que Ana nem está olhando no momento, então a linha de canal de voz com avatares inline na lista de canais, §6, não está nem visível) — por isso a miniatura de participantes fica na própria barra, não só o nome do canal.

**Anel de fala ativa — visível mesmo recolhido**, nos avatares da stack: mesmo token de cor/animação do anel expandido (2.3, `presence-online` com pulse, §5.4), só escalado para o tamanho menor (anel de 1.5px em vez de 2px). Mantém a regra de acessibilidade já estabelecida — forma + movimento, não só cor — mesmo em miniatura; reduzir a fala ativa a uma cor estática nesse tamanho quebraria essa regra. O badge de overflow "+N" nunca recebe anel, por não representar uma pessoa específica.
...
**Topologia (premissa 9):** câmera vai pelo **mesh direto da voz**, igual ao áudio — cada participante manda seu vídeo pra cada outro. A árvore de multicast de `CLAUDE.md:12-19` é para transmissão a audiência (compartilhamento de tela, 2.4), não para uma chamada de poucos. Consequência que a interface precisa admitir: com muita gente de câmera ligada o upload de cada um cresce linearmente, então **acima de 6 câmeras simultâneas** a grade para de aceitar novas e o botão fica desabilitado com o motivo ("Muitas câmeras ligadas nesta chamada — o áudio tem prioridade"). Número ilustrativo, coerente com o "≤4-5" que o `CLAUDE.md` usa como limiar de estrela.
...
**Navegação:** aberta pelo avatar de Ana no rodapé do rail (§8, 1.1); fecha voltando pro shell na comunidade que estava ativa antes.
...
4. Barra de chamada persistente aparece na base da sidebar.
...
2. Barra de chamada persistente continua visível ("Sala de Estudos — 3 participantes" + controles rápidos).
...
3. Rafael confirma → a chamada encerra para todo mundo (barra de chamada persistente some, como em C11) · o canal some da lista · quem estava com ele aberto cai no primeiro canal de texto · entrada no log de auditoria.
**Estados intermediários:** menu aberto → confirmando → processando (~500ms) → concluído.
...
Painéis (membros, busca, thread, configurações) e modais abrem **por cima** dessa pilha, sempre em tela cheia, sempre com "voltar"/"fechar" explícito — nunca overlay flutuante pequeno em Mobile (não há espaço). Popover de perfil (1.4) vira bottom sheet. A barra de chamada persistente fica fixada no rodapé da viewport **independente de qual das 3 telas está em foco** — é a única coisa que sobrevive à navegação sequencial.
...
- **Composer (§6, §9 2.1)** — textarea nativo que cresce até 40% da viewport antes de rolar, placeholder "Conversar em #canal", `Enter` envia e `Shift+Enter` quebra linha. Botão de anexo abre o seletor do SO, o arquivo escolhido vira chip com nome, tamanho e "x", e arquivo acima do teto ilustrativo de 8 GB dá erro inline "Arquivo muito grande" antes de tentar enviar (§11, C9).
...
- **Qualidade é de quem assiste; parar é de quem apresenta.** §9 (2.4) lista as duas ações juntas sem separar por papel. Ajustar a própria recepção não afeta ninguém, então ficou disponível para todos; encerrar a transmissão, só para o apresentador.
...
- **Compartilhamentos simultâneos (§18) não entraram** — uma sessão de compartilhamento por chamada. É um edge case sem fluxo próprio, e a grade de tiles grandes que ele pede duplicaria toda a lógica de topologia por tile.
...
- **6 · §9, 2.3.2** — tile troca o avatar por superfície de vídeo simulada (`animate-camera-drift`), espelhada só para quem se vê, e o botão de câmera fica inativo com o motivo acima de 6 câmeras. Conexão degradada derruba o vídeo e devolve o avatar: o áudio tem prioridade.
