...
**Desktop e Tablet** (ambos mantêm a coluna da esquerda, §16):
...
**Mobile** (<640px):
- Container: largura total da viewport, 64px de altura, fixo no rodapé — acima de qualquer uma das 3 telas empilhadas (§16), sempre visível independente de qual está em foco. Fundo `surface-elevated`, `border-subtle` no topo, padding 12px. A barra de usuário não está visível junto do conteúdo neste breakpoint, então esta barra continua sendo a superfície da chamada, com mudo incluído.
