...
| # | Item | O que só o humano tem | Referência |
|---|---|---|---|
| B40 | **Áudio do compartilhamento de tela só existe no Windows, e a promessa "só desta janela" não foi medida.** O `audio: 'loopback'` do Electron é do Windows; no Linux a captura de tela sobe muda e a UI diz isso. O Modo Música saiu desta linha em §116: no Linux ele usa o monitor de reprodução do sistema (`acharMonitorDeSistema`, §17.5 item 7). O recorte por janela continua do Chromium (`GetDisplayMediaWindowAudioCapture`) e o pedido é feito, mas nada aqui mediu se ele é honrado | Uma máquina Windows, para conferir que o som transmitido é o do aplicativo e não o da máquina | `app/src/main/captura.ts`, B39, §116 |
