...
| B68 | **Compartilhar a tela numa conversa direta não é derivável de §31.15.** §31.15 abre com "Vale §17.2 sem alteração" — e §17.2 põe voz e **câmera** na mesma malha, o que tornou a câmera pura derivação (§112, implementada sem tocar no núcleo nem na tabela fechada de §31.8). A **tela** não tem esse apoio: §17.5 é uma estrela **autorizada pelo host** (`shareStart`/`shareJoin`/`shareLeave`/`shareQuality` em §16.2, `share.started`/`share.health`/`share.viewersChanged`/`share.failed` na tabela fechada de §16.3, tudo com `sessionId` e ticket), e os **cinco passos** do laço de saúde passam todos pelo host: ele registra o perfil pedido por espectador, recebe `share.report`, consolida, decide a degradação e emite `share.health`. §31.15 remove o host, o ticket, o roster e a revogação, e **não menciona §17.5 em linha nenhuma** da tabela de remoções. Duas coisas ficam sem fonte: (a) quem cria e encerra a sessão, a quem `share.setQuality` pede e quem decide a degradação, sem host; (b) **B41 vira bloqueante** — nada no fio distingue tela de câmera, e numa DM não existe `share.join`, então a regra 3 de `classificarVideo` fica sem entrada e não há estado local a conquistar. Enquanto a câmera for a única trilha de vídeo possível a classificação é total por construção; admitir a tela sem campo no fio a torna **inverificável**, que é o critério de B66/B67 | A decisão de texto: ou §31.15 ganha uma linha declarando que numa dupla a estrela de §17.5 **é** a malha de dois (e então some sessão, ticket, roster e o laço de saúde inteiro, restando dizer quem aplica `maxBitrate` a pedido de quem), ou a tela fica fora da DM no v1 e isso é dito. Nos dois casos B41 precisa de resposta no fio para a DM. Definido isso, a implementação é do agente | §31.15, §17.5, §16.2, §16.3, §112, **B41** |
...
identidade), **B66**, **B67** e **B68** de texto normativo, e **B4** — a medida em rede real,
